...
times = pd.date_range('2023-03-20 5:17', freq='h', periods=1, tz='Etc/GMT+7')
...
import pandas as pd 
df = pd.read_csv("Data_irradiance_solaire.csv", sep = ",")
lambdda = df["lambda (nm)"].to_numpy()
Dsp = df["Densite spectrale (W/m2/nm)"].to_numpy()

plt.figure()
plt.grid()
plt.plot(lambdda,Dsp, "black")
plt.xlabel("Longueur d'onde $\lambda$ (nm)")
plt.ylabel("Puissance surfacique par unité de\n  longueur d'onde($W\cdot m^{-2}\cdot nm^{-1}$)")
plt.title("Données du National Renewable Energy Laboratory")
plt.savefig("EmissionDuSoleil.png", dpi=300)